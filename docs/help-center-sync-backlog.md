...
| 2026-09-14 | operations | 一键系统更新改为可恢复进度的后台任务 | 设置页安装当前受支持 Debian/Ubuntu 发行版的全部常规软件包更新；关闭或刷新页面不会中断，可重新显示当前阶段并阻止重复更新。完成后检查软件包、Nginx 配置和核心服务；检查失败会提示联系项目支持，不会自动降级软件包 | `docs/features/operations-and-settings.md`、ADR-0043 | 待同步 |
